...
- (void)datePhotoViewCell:(HXDatePhotoViewCell *)cell didSelectBtn:(UIButton *)selectBtn {
    ...
    if (!selectBtn.selected) {
        ...
        for (HXPhotoModel *model in [self.manager selectedArray]) {
            model.selectIndexStr = [NSString stringWithFormat:@"%ld",(long)(index + 1)];
...
- (void)datePhotoPreviewControllerDidSelect:(HXDatePhotoPreviewViewController *)previewController model:(HXPhotoModel *)model {
    ...
    if (!model.selected) {
        ...
        for (HXPhotoModel *subModel in [self.manager selectedArray]) {
            subModel.selectIndexStr = [NSString stringWithFormat:@"%ld",(long)(index + 1)];
...
- (void)setVideoCount:(NSInteger)videoCount {
    ...
    if (self.photoCount > 0 && videoCount > 0) {
        ...
        self.titleLb.text = [NSString stringWithFormat:@"%ld %@、%ld %@",(long)self.photoCount,[NSBundle hx_localizedStringForKey:photoStr],(long)videoCount,[NSBundle hx_localizedStringForKey:videoStr]];
        ...
        self.titleLb.text = [NSString stringWithFormat:@"%ld %@",(long)self.photoCount,[NSBundle hx_localizedStringForKey:photoStr]];
    }else {
        NSString *videoStr;
        if (videoCount > 1) {
            videoStr = @"Videos";
        ...
        self.titleLb.text = [NSString stringWithFormat:@"%ld %@",(long)videoCount,
        ...
        if (self.manager.configuration.doneBtnShowDetail) {
            if (!self.manager.configuration.selectTogether) {
                if (self.manager.selectedPhotoCount > 0) {
                    [self.doneBtn setTitle:[NSString stringWithFormat:@"%@(%ld/%ld)",[NSBundle hx_localizedStringForKey:@"完成"],(long)selectCount,(long)self.manager.configuration.photoMaxNum] forState:UIControlStateNormal];
                }else {
                    [self.doneBtn setTitle:[NSString stringWithFormat:@"%@(%ld/%ld)",[NSBundle hx_localizedStringForKey:@"完成"],(long)selectCount,(long)self.manager.configuration.videoMaxNum] forState:UIControlStateNormal];
                }
            }else {
                [self.doneBtn setTitle:[NSString stringWithFormat:@"%@(%ld/%ld)",[NSBundle hx_localizedStringForKey:@"完成"],(long)selectCount,(long)self.manager.configuration.maxNum] forState:UIControlStateNormal];
            }
        }else {
