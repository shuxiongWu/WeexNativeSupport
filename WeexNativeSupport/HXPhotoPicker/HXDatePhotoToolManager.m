...
- (void)getCurrentModelImage {
    ...
    if (model.asset) {
        ...
        if (model.networkPhotoUrl) {
            ...
            if (model.downloadError) {
//#if __has_include(<SDWebImage/UIImageView+WebCache.h>) || __has_include("UIImageView+WebCache.h")
//                SDWebImageDownloaderoken = [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:model.networkPhotoUrl options:0 progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished) {
//                    if (!error && image) {
//                        model.thumbPhoto = image;
//                        model.previewPhoto = image;
//                        [weakSelf.imageArray addObject:model.thumbPhoto];
