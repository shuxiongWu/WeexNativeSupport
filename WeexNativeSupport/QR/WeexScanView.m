...
- (void)drawRect:(CGRect)rect{
    ...
    UILabel * descLabel  = [[UILabel alloc] initWithFrame:CGRectMake(0, self.scanRetangleRect.origin.y + self.scanRetangleRect.size.height + 15, rect.size.width, 20)];
    descLabel.font = [UIFont systemFontOfSize:14];
    descLabel.textAlignment = NSTextAlignmentCenter;
    descLabel.textColor = [UIColor whiteColor];
    descLabel.text = self.descriptionString ?: @"将二维码放入框内，即可自动扫描";
...
- (UIButton *)flashBtn{
    if (_flashBtn == nil) {
        ...
        [_flashBtn setImage:[WeexPublicTool wx_imageNamed:@"scanFlashlight"] forState:UIControlStateNormal];
